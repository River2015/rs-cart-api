...
export class CartService {
  ...
  async createByUserId(userId: string): Promise<Cart> {
    const query = `INSERT INTO carts (user_id, created_at, updated_at, status) VALUES ('${userId}', '${new Date()}', '${new Date()}', '${CartStatus.OPEN}') RETURNING id`;
    const res = await this.db.runQuery<Cart>(query);
    return res[0];
  }

  async findOrCreateByUserId(userId: string, status?: CartStatus): Promise<Cart> {
